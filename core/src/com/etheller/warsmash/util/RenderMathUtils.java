...
public enum RenderMathUtils {
    ...
    public static void perspective(final Matrix4 out, final float fovy, final float aspect, final float near,
                                   final float far) {
    ...
    public static void ortho(final Matrix4 out, final float left, final float right, final float bottom,
    ...
    public static void unproject(final Vector3 out, final Vector3 v, final Matrix4 inverseMatrix,
    ...
    public static void slerp(final float[] out, final float[] a, final float[] b, final float t) {
    ...
    public static void sqlerp(final float[] out, final float[] a, final float[] b, final float[] c, final float[] d,
                              final float t) {
    ...
    public static Buffer wrapPairs(final float[][] quadVertices) {
        final FloatBuffer wrapper = ByteBuffer.allocateDirect(quadVertices.length * 8).order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        for (float[] quadVertex : quadVertices) {
            for (int j = 0; j < 2; j++) {
                wrapper.put(quadVertex[j]);
            }
        }
        wrapper.clear();
        return wrapper;
