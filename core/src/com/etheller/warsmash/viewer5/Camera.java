...
public class Camera {
    ...
    public final Quaternion inverseRotation;
    ...
    public Camera() {
        ...
        // are the unit axes
        this.vectors = new Vector3[]{new Vector3(-1, -1, 0), new Vector3(-1, 1, 0), new Vector3(1, 1, 0), new Vector3(1, -1, 0), new Vector3(1, 0, 0), new Vector3(0, 1, 0), new Vector3(0, 0, 1)};
        ...
        this.billboardedVectors = new Vector3[]{new Vector3(), new Vector3(), new Vector3(), new Vector3(), new Vector3(), new Vector3(), new Vector3()};

        // Left, right, top, bottom, near, far
        this.planes = new Vector4[]{new Vector4(), new Vector4(), new Vector4(), new Vector4(), new Vector4(), new Vector4()};
    ...
    public void update() {
        if (this.dirty) {
            ...
            if (this.isPerspective) {
                RenderMathUtils.perspective(projectionMatrix, this.fov, this.aspect, this.nearClipPlane, this.farClipPlane);
            } else {
                RenderMathUtils.ortho(projectionMatrix, this.leftClipPlane, this.rightClipPlane, this.bottomClipPlane, this.topClipPlane, this.nearClipPlane, this.farClipPlane);
            }
    ...
    public void worldToCamera(final Vector3 out, final Vector3 v) {
        out.set(v).prj(this.viewMatrix);
    }

    public void worldToScreen(final Vector2 out, final Vector3 v) {
        ...
        out.y = ((Gdx.graphics.getHeight() - viewport.y - viewport.height) + (viewport.height)) - Math.round(((vectorHeap.y + 1) / 2) * viewport.height);

    }

    public void screenToWorldRay(final float[] out, final Vector2 v) {
