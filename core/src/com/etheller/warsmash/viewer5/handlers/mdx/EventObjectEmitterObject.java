...
public class EventObjectEmitterObject extends GenericObject implements EmitterObject {
    private static final LoadGenericCallback mappedDataCallback = data -> {
        if (data == null) {
    ...
    public float volume;
    public final List<Sound> decodedBuffers = new ArrayList<>();
...
//					this.internalModel.whenLoaded((model) => this.ok = model.ok)
                ...
                if (viewer.audioEnabled) {
                    ...
                    if (animSoundsRow != null) {
                        ...
                        float maxDistance = getFloat(animSoundsRow, "MaxDistance");
