...
public class Camera extends AnimatedObject {
    ...
    public void getPositionTranslation(final float[] out, final int sequence, final int frame, final int counter) {
        this.getVectorValue(out, AnimationMap.KCTR.getWar3id(), sequence, frame, counter,
                RenderMathUtils.FLOAT_VEC3_ZERO);
    }

    public void getTargetTranslation(final float[] out, final int sequence, final int frame, final int counter) {
        this.getVectorValue(out, AnimationMap.KTTR.getWar3id(), sequence, frame, counter,
                RenderMathUtils.FLOAT_VEC3_ZERO);
    }
