 ...
 * <a href="https://github.com/d07RiV/wc3data/blob/3435e9728663825d892693318d0a0bb823dfad8c/src/mdx/viewer/handlers/w3x/splatmodel.js">...</a>
 * <p>
...
public class SplatModel implements Comparable<SplatModel> {
    ...
    private void loadBatches(final GL30 gl, final float[] centerOffset) {
        ...
        for (int idx = 0; idx < instances; ++idx) {
            ...
            if (this.splatInstances != null) {
                ...
                while (Objects.requireNonNull(splatMover).indices.size() < maxPossibleFaces) {
                    final int[] indexArray = new int[]{start, start, start, start, start, start};
                    indices.add(indexArray);
                    splatMover.indices.add(indexArray);
                }
            }
    ...
    public static final class SplatMover {
        ...
        public void destroy(final GL30 gl, final float[] centerOffset) {
            ...
            Objects.requireNonNull(this.splatModel.splatInstances).remove(this.index);
