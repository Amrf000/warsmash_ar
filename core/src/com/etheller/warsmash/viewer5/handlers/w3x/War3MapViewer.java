...
public class War3MapViewer extends AbstractMdxModelViewer {
    ...
    private static final List<String> ORIGIN_STRING_LIST = Collections.singletonList("origin");
    ...
    public final WorldScene worldScene;
    public final MappedData terrainData = new MappedData();
    public final MappedData cliffTypesData = new MappedData();
    public final MappedData waterData = new MappedData();
    public final MappedData doodadsData = new MappedData();
    public final MappedData doodadMetaData = new MappedData();
    public final MappedData destructableMetaData = new MappedData();
    public final List<RenderDoodad> doodads = new ArrayList<>();
    public final List<TerrainDoodad> terrainDoodads = new ArrayList<>();
    public final MappedData unitsData = new MappedData();
    public final MappedData unitMetaData = new MappedData();
    public final List<RenderWidget> widgets = new ArrayList<>();
    public final List<RenderUnit> units = new ArrayList<>();
    public final List<RenderEffect> projectiles = new ArrayList<>();
    public final int renderPathing = 0;
    public final int renderLighting = 1;
    public final List<RenderWidget> selected = new ArrayList<>();
    // for World Editor, I think
    public final Vector2[] startLocations = new Vector2[WarsmashConstants.MAX_PLAYERS];
    ...
    public War3MapViewer(final DataSource dataSource, final CanvasProvider canvas, final War3MapConfig mapConfig, final GameTurnManager gameTurnManager) {
    ...
    public static MdxModel loadModelMdx(final DataSource dataSource, final ModelViewer modelViewer, final String path, final PathSolver pathSolver, final Object solverParams) {
    ...
    public void loadSLKs(final WorldEditStrings worldEditStrings) throws IOException {
        final GenericResource terrain = loadMapGeneric("TerrainArt\\Terrain.slk", FetchDataTypeName.SLK, stringDataCallback);
        final GenericResource cliffTypes = loadMapGeneric("TerrainArt\\CliffTypes.slk", FetchDataTypeName.SLK, stringDataCallback);
        final GenericResource water = loadMapGeneric("TerrainArt\\Water.slk", FetchDataTypeName.SLK, stringDataCallback);
        ...
        final GenericResource doodads = loadMapGeneric("Doodads\\Doodads.slk", FetchDataTypeName.SLK, stringDataCallback);
        final GenericResource doodadMetaData = loadMapGeneric("Doodads\\DoodadMetaData.slk", FetchDataTypeName.SLK, stringDataCallback);
        final GenericResource destructableData = loadMapGeneric("Units\\DestructableData.slk", FetchDataTypeName.SLK, stringDataCallback);
        ...
        final GenericResource unitData = loadMapGeneric("Units\\UnitData.slk", FetchDataTypeName.SLK, stringDataCallback);
        final GenericResource unitUi = loadMapGeneric("Units\\unitUI.slk", FetchDataTypeName.SLK, stringDataCallback);
        final GenericResource itemData = loadMapGeneric("Units\\ItemData.slk", FetchDataTypeName.SLK, stringDataCallback);
        final GenericResource unitMetaData = loadMapGeneric("Units\\UnitMetaData.slk", FetchDataTypeName.SLK, stringDataCallback);
        ...
        try (InputStream terrainSlkStream = this.dataSource.getResourceAsStream("UI\\SoundInfo\\UnitCombatSounds.slk")) {
        ...
        DataTable unitGlobalStrings = new DataTable(worldEditStrings);
        try (InputStream miscDataTxtStream = this.dataSource.getResourceAsStream("Units\\UnitGlobalStrings.txt")) {
            unitGlobalStrings.readTXT(miscDataTxtStream, true);
        }
        final Element categories = unitGlobalStrings.get("Categories");
    ...
    private Color parseColor(final Element selectionCircleData, final String field) {
        return new Color(selectionCircleData.getFieldFloatValue(field, 1) / 255f, selectionCircleData.getFieldFloatValue(field, 2) / 255f, selectionCircleData.getFieldFloatValue(field, 3) / 255f, selectionCircleData.getFieldFloatValue(field, 0) / 255f);
    }

    public GenericResource loadMapGeneric(final String path, final FetchDataTypeName dataType, final LoadGenericCallback callback) {
        if (this.mapMpq == null) {
    ...
    public void loadMap(final War3Map war3Map, final War3MapW3i w3iFile, final int localPlayerIndex) throws IOException {
        ...
        try {
            ...
            if (WarsmashConstants.FIX_FLAT_FILES_TILESET_LOADING) {
                tilesetSource = new CompoundDataSource(Arrays.asList(compoundDataSource, new SubdirDataSource(compoundDataSource, tileset + ".mpq/")));
            } else {
                try (InputStream mapStream = compoundDataSource.getResourceAsStream(tileset + ".mpq")) {
                    if (mapStream == null) {
                        tilesetSource = new CompoundDataSource(Arrays.asList(compoundDataSource, new SubdirDataSource(compoundDataSource, tileset + ".mpq/"), new SubdirDataSource(compoundDataSource, "_tilesets/" + tileset + ".w3mod/")));
                    } else {
                        ...
                        tilesetSource = new CompoundDataSource(Arrays.asList(compoundDataSource, internalMpqContentsDataSource));
                    }
        ...
        this.terrain = new Terrain(terrainData, terrainPathing, w3iFile, this.webGL, this.dataSource, this.worldEditStrings, this, this.worldEditData);
        ...
        final MdxModel confirmation = (MdxModel) load("UI\\Feedback\\Confirmation\\Confirmation.mdx", PathSolver.DEFAULT, null);
        ...
        this.simulation = new CSimulation(this.mapConfig, this.miscData, this.allObjectData.getUnits(), this.allObjectData.getItems(), this.allObjectData.getDestructibles(), this.allObjectData.getAbilities(), new SimulationRenderController() {
            private final Map<String, UnitSound> keyToCombatSound = new HashMap<>();

            @Override
            public CAttackProjectile createAttackProjectile(final CSimulation simulation, final float launchX, final float launchY, final float launchFacing, final CUnit source, final CUnitAttackMissile unitAttack, final AbilityTarget target, final float damage, final int bounceIndex, final CUnitAttackListener attackListener) {
                ...
                final float sinFacing = (float) Math.sin(launchFacing);
                final float cosFacing = (float) Math.cos(launchFacing);
                ...
                final float height = War3MapViewer.this.terrain.getGroundHeight(x, y) + source.getFlyHeight() + projectileLaunchZ;
                final CAttackProjectile simulationAttackProjectile = new CAttackProjectile(x, y, projectileSpeed, target, source, damage, unitAttack, bounceIndex, attackListener);
                ...
                final RenderAttackProjectile renderAttackProjectile = new RenderAttackProjectile(simulationAttackProjectile, modelInstance, height, projectileArc, War3MapViewer.this);
            ...
            @Override
            public void createInstantAttackEffect(final CSimulation cSimulation, final CUnit source, final CUnitAttackInstant unitAttack, final CWidget target) {
                final War3ID typeId = source.getTypeId();
                ...
                final float height = War3MapViewer.this.terrain.getGroundHeight(targetX, targetY) + target.getFlyHeight() + target.getImpactZ();
                ...
                modelInstance.setScene(War3MapViewer.this.worldScene);
                War3MapViewer.this.projectiles.add(new RenderAttackInstant(modelInstance, War3MapViewer.this, angleToTarget));
            }

            @Override
            public void spawnDamageSound(final CWidget damagedDestructable, final String weaponSound, final String armorType) {
                ...
                if (combatSound == null) {
                    combatSound = UnitSound.create(War3MapViewer.this.dataSource, War3MapViewer.this.unitCombatSoundsTable, weaponSound, armorType);
                    this.keyToCombatSound.put(key, combatSound);
                }
                combatSound.play(War3MapViewer.this.worldScene.audioContext, damagedDestructable.getX(), damagedDestructable.getY(), damagedWidget.getZ());
            }

            @Override
            public void spawnUnitConstructionSound(final CUnit constructingUnit, final CUnit constructedStructure) {
                final UnitSound constructingBuilding = War3MapViewer.this.uiSounds.getSound(War3MapViewer.this.gameUI.getSkinField("ConstructingBuilding"));
                if (constructingBuilding != null) {
                    constructingBuilding.playUnitResponse(War3MapViewer.this.worldScene.audioContext, War3MapViewer.this.unitToRenderPeer.get(constructedStructure));
                }
            }
...
//						final RenderUnit renderUnit = War3MapViewer.this.unitToRenderPeer.get(unit);
                final MutableGameObject upgrade = War3MapViewer.this.allObjectData.getUnits().get(upgradeIdType);

                // TODO this should be behind some auto lookup so it isn't copied from
                // RenderUnit class:
                final String originalRequiredAnimationNames = War3MapViewer.this.allObjectData.getUnits().get(unit.getTypeId()).getFieldAsString(RenderUnit.ANIM_PROPS, 0);
                TokenLoop:
            ...
            public void removeDestructable(final CDestructable dest) {
                ...
                if (renderPeer.walkableBounds != null) {
                    War3MapViewer.this.walkableObjectsTree.remove((MdxComplexInstance) renderPeer.instance, renderPeer.walkableBounds);
                }
            }

            @Override
            public BufferedImage getBuildingPathingPixelMap(final War3ID rawcode) {
                return War3MapViewer.this.getBuildingPathingPixelMap(War3MapViewer.this.allObjectData.getUnits().get(rawcode));
            }

            @Override
            public BufferedImage getDestructablePathingDeathPixelMap(final War3ID rawcode) {
                return War3MapViewer.this.getDestructablePathingDeathPixelMap(War3MapViewer.this.allObjectData.getDestructibles().get(rawcode));
            }

            @Override
            public BufferedImage getDestructablePathingPixelMap(final War3ID rawcode) {
                return War3MapViewer.this.getDestructablePathingPixelMap(War3MapViewer.this.allObjectData.getDestructibles().get(rawcode));
            }

            @Override
            public void spawnUnitConstructionFinishSound(final CUnit constructedStructure) {
                final UnitSound constructingBuilding = War3MapViewer.this.uiSounds.getSound(War3MapViewer.this.gameUI.getSkinField("JobDoneSound"));
                final RenderUnit renderUnit = War3MapViewer.this.unitToRenderPeer.get(constructedStructure);
                if ((constructingBuilding != null) && (renderUnit.getSimulationUnit().getPlayerIndex() == War3MapViewer.this.localPlayerIndex)) {
                    constructingBuilding.play(War3MapViewer.this.worldScene.audioContext, constructedStructure.getX(), constructedStructure.getY(), renderUnit.getZ());
                }
            }

            @Override
            public CUnit createUnit(final CSimulation simulation, final War3ID typeId, final int playerIndex, final float x, final float y, final float facing) {
                return (CUnit) War3MapViewer.this.createNewUnit(War3MapViewer.this.allObjectData, typeId, x, y, playerIndex, playerIndex, (float) Math.toRadians(facing));
            }

            @Override
            public CDestructable createDestructable(final War3ID typeId, final float x, final float y, final float facing, final float scale, final int variation) {
                return createDestructableZ(typeId, x, y, Math.max(War3MapViewer.this.getWalkableRenderHeight(x, y), War3MapViewer.this.terrain.getGroundHeight(x, y)), facing, scale, variation);
            }

            @Override
            public CDestructable createDestructableZ(final War3ID typeId, final float x, final float y, final float z, final float facing, final float scale, final int variation) {
                ...
                final RenderDestructable newDestructable = War3MapViewer.this.createNewDestructable(typeId, row, variation, location3d, (float) Math.toRadians(facing), (short) 100, scale3d);
            ...
            public CItem createItem(final CSimulation simulation, final War3ID typeId, final float x, final float y) {
                return (CItem) War3MapViewer.this.createNewUnit(War3MapViewer.this.allObjectData, typeId, x, y, -1, -1, (float) Math.toRadians(War3MapViewer.this.simulation.getGameplayConstants().getBuildingAngle()));
            }
            ...
            public void spawnBuildingDeathEffect(final CUnit source) {
                ...
                if (renderUnit.specialArtModel != null) {
                    final MdxComplexInstance modelInstance = (MdxComplexInstance) renderUnit.specialArtModel.addInstance();
                    modelInstance.setTeamColor(source.getPlayerIndex());
                    modelInstance.setLocation(renderUnit.location);
                    modelInstance.setScene(War3MapViewer.this.worldScene);
                    SequenceUtils.randomBirthSequence(modelInstance);
                    War3MapViewer.this.projectiles.add(new RenderAttackInstant(modelInstance, War3MapViewer.this, (float) Math.toRadians(renderUnit.getSimulationUnit().getFacing())));
                }
            }
            ...
            public void heroRevived(final CUnit source) {
                ...
                final MutableGameObject row = War3MapViewer.this.allObjectData.getUnits().get(source.getTypeId());
                ...
                if ((unitShadow != null) && !"_".equals(unitShadow)) {
                    ...
                    if (War3MapViewer.this.mapMpq.has(texture)) {
                        ...
                        renderUnit.shadow = War3MapViewer.this.terrain.addUnitShadowSplat(texture, x, y, x + shadowWidth, y + shadowHeight, 3, 0.5f);
                    } else {
                        final String textureFallback = "ReplaceableTextures\\Shadows\\" + unitShadow + ".dds";
                        if (War3MapViewer.this.mapMpq.has(textureFallback)) {
                            final float x = unitX - shadowX;
                            final float y = unitY - shadowY;
                            renderUnit.shadow = War3MapViewer.this.terrain.addUnitShadowSplat(textureFallback, x, y, x + shadowWidth, y + shadowHeight, 3, 0.5f);
                        }
                    }
                }
            }
            ...
            @Override
            public void spawnSpellEffectOnUnit(final CUnit unit, final War3ID alias, final CEffectType effectType) {
                final RenderSpellEffect spellEffect = spawnSpellEffectOnUnitEx(unit, alias, effectType, 0);
                Objects.requireNonNull(spellEffect).setKillWhenDone(true);
            }

            @Override
            public SimulationRenderComponent spawnSpellEffectOnUnit(final CUnit unit, final War3ID alias, final CEffectType effectType, final int index) {
                ...
                return () -> specialEffect.setAnimations(RenderSpellEffect.DEATH_ONLY, true);
            }

            private RenderSpellEffect spawnSpellEffectOnUnitEx(final CUnit unit, final War3ID alias, final CEffectType effectType, final int index) {
                ...
                return addSpecialEffectTarget(modelPath, unit, attachmentPoint);
            }
            ...
            public SimulationRenderComponent createSpellEffectOverDestructable(final CUnit source, final CDestructable target, final War3ID alias, final float artAttachmentHeight) {
                ...
                if (spawnedEffectModel != null) {
                    ...
                    modelInstance.setScene(War3MapViewer.this.worldScene);
                    final RenderSpellEffect renderAttackInstant = new RenderSpellEffect(modelInstance, War3MapViewer.this, 0, RenderSpellEffect.STAND_ONLY);
                    ...
                    return () -> renderAttackInstant.setAnimations(RenderSpellEffect.DEATH_ONLY, true);
                }
                return null;
            }

            @Override
            public void createSpellEffectFacing(final War3ID alias, final float harvestStandX, final float harvestStandY, final float angle) {
                final AbilityUI abilityUI = War3MapViewer.this.abilityDataUI.getUI(alias);
                ...
                if (spawnedEffectModel != null) {
                    final MdxComplexInstance modelInstance = (MdxComplexInstance) spawnedEffectModel.addInstance();
                    modelInstance.setLocation(harvestStandX, harvestStandY, War3MapViewer.this.terrain.getGroundHeight(harvestStandX, harvestStandY));
                    modelInstance.setScene(War3MapViewer.this.worldScene);
                    final RenderSpellEffect renderAttackInstant = new RenderSpellEffect(modelInstance, War3MapViewer.this, angle, RenderSpellEffect.DEFAULT_ANIMATION_QUEUE);
                    renderAttackInstant.setAnimations(RenderSpellEffect.STAND_ONLY, false);
                    War3MapViewer.this.projectiles.add(renderAttackInstant);
                }
            }
            ...
            public void unitUpdatedType(final CUnit simulationUnit, final War3ID typeId) {
                ...
                final float unitZ = Math.max(getWalkableRenderHeight(unitX, unitY), War3MapViewer.this.terrain.getGroundHeight(unitX, unitY)) + simulationUnit.getFlyHeight();

                UnitSoundset soundset;
                ...
                if (unitSoundset == null) {
                    unitSoundset = new UnitSoundset(War3MapViewer.this.dataSource, War3MapViewer.this.unitAckSoundsTable, soundName);
                    War3MapViewer.this.soundsetNameToSoundset.put(soundName, unitSoundset);
                }
                soundset = unitSoundset;

                renderPeer.resetRenderUnit(War3MapViewer.this, model, row, unitX, unitY, unitZ, renderPeer.playerIndex, soundset, portraitModel, simulationUnit, typeData, specialArtModel, buildingShadowInstance, War3MapViewer.this.selectionCircleScaleFactor, typeData.getAnimationWalkSpeed(), typeData.getAnimationRunSpeed(), typeData.getScalingValue());
            }

            @Override
            public void spawnGainResourceTextTag(final CUnit gainingUnit, final ResourceType resourceType, final int amount) {
                final RenderUnit renderPeer = War3MapViewer.this.unitToRenderPeer.get(gainingUnit);
                switch (resourceType) {
                    case FOOD:
                        throw new IllegalArgumentException();
                    case GOLD:
                        War3MapViewer.this.textTags.add(new TextTag(new Vector3(renderPeer.location), "+" + amount, PLACEHOLDER_GOLD_COLOR));
                        break;
                    case LUMBER:
                        War3MapViewer.this.textTags.add(new TextTag(new Vector3(renderPeer.location), "+" + amount, PLACEHOLDER_LUMBER_COLOR));
                        break;
                }
            }
            ...
            public void spawnUIUnitGetItemSound(final CUnit cUnit, final CItem item) {
                ...
                if (localPlayerIndex == renderPeer.getSimulationUnit().getPlayerIndex()) {
                    War3MapViewer.this.uiSounds.getSound("ItemGet").play(War3MapViewer.this.worldScene.audioContext, renderPeer.getX(), renderPeer.getY(), renderPeer.getZ());
                }
            }
            ...
            public void spawnUIUnitDropItemSound(final CUnit cUnit, final CItem item) {
                final RenderUnit renderPeer = War3MapViewer.this.unitToRenderPeer.get(cUnit);
                if (localPlayerIndex == renderPeer.getSimulationUnit().getPlayerIndex()) {
                    War3MapViewer.this.uiSounds.getSound("ItemDrop").play(War3MapViewer.this.worldScene.audioContext, renderPeer.getX(), renderPeer.getY(), renderPeer.getZ());
                }
            }
            ...
            public void spawnAbilitySoundEffect(final CUnit caster, final War3ID alias) {
                ...
                if (abilityUi.getEffectSound() != null) {
                    War3MapViewer.this.uiSounds.getSound(abilityUi.getEffectSound()).play(War3MapViewer.this.worldScene.audioContext, renderPeer.getX(), renderPeer.getY(), renderPeer.getZ());
                }
            }
            ...
            public void loopAbilitySoundEffect(final CUnit caster, final War3ID alias) {
                ...
                if (abilityUi.getEffectSoundLooped() != null) {
                    War3MapViewer.this.uiSounds.getSound(abilityUi.getEffectSoundLooped()).play(War3MapViewer.this.worldScene.audioContext, renderPeer.getX(), renderPeer.getY(), renderPeer.getZ());
                }
            }
    ...
    private void loadDoodadsAndDestructibles(final Warcraft3MapObjectData modifications, final War3MapW3i w3iFile) throws IOException {
        applyModificationFile(this.doodadsData, this.doodadMetaData, modifications.getDoodads(), WorldEditorDataType.DOODADS);
        applyModificationFile(this.doodadsData, this.destructableMetaData, modifications.getDestructibles(), WorldEditorDataType.DESTRUCTIBLES);
        ...
        for (final com.etheller.warsmash.parsers.w3x.doo.Doodad doodad : doo.getDoodads()) {
            ...
            createDestructableOrDoodad(doodadId, modifications, doodadVariation, location, facingRadians, lifePercent, scale);
        }
        ...
        for (final com.etheller.warsmash.parsers.w3x.doo.TerrainDoodad doodad : doo.getTerrainDoodads()) {
            ...
            if ((pathingTexture != null) && (pathingTexture.length() > 0) && !"_".equals(pathingTexture)) {
                ...
                if (pathingTextureImage == null) {
                    if (this.mapMpq.has(pathingTexture)) {
                        try {
                            pathingTextureImage = TgaFile.readTGA(pathingTexture, this.mapMpq.getResourceAsStream(pathingTexture));
            ...
            this.terrainDoodads.add(new TerrainDoodad(this, model, row, doodad, Objects.requireNonNull(pathingTextureImage)));
        }
    ...
    private void createDoodad(final MutableGameObject row, final int doodadVariation, final float[] location, final float facingRadians, final float[] scale) {
        ...
        final RenderDoodad renderDoodad = new RenderDoodad(this, model, row, location, scale, facingRadians, maxPitch, maxRoll, defScale);
        renderDoodad.instance.uniformScale(defScale);
        this.doodads.add(renderDoodad);
    }

    private RenderDestructable createNewDestructable(final War3ID doodadId, final MutableGameObject row, final int doodadVariation, final float[] location, final float facingRadians, final short lifePercent, final float[] scale) {
        ...
        final CDestructable simulationDestructable = this.simulation.internalCreateDestructable(row.getAlias(), x, y, destructablePathing, destructablePathingDeath);
        ...
        final RenderDestructable renderDestructable = new RenderDestructable(this, model, row, location, scale, facingRadians, selectionScale, maxPitch, maxRoll, lifePercent, destructableShadow, simulationDestructable);
        if (row.readSLKTagBoolean("walkable")) {
            ...
            final Rectangle renderDestructableBounds = getRotatedBoundingBox(x, y, facingRadians, boundingBox);
    ...
    private void createDestructableOrDoodad(final War3ID doodadId, final Warcraft3MapObjectData modifications, final int doodadVariation, final float[] location, final float facingRadians, final short lifePercent, final float[] scale) {
    ...
    private Rectangle getRotatedBoundingBox(final float x, final float y, final float angle, final BoundingBox boundingBox) {
    ...
    private void applyModificationFile(final MappedData doodadsData2, final MappedData doodadMetaData2, final MutableObjectData destructibles, final WorldEditorDataType dataType) {
        // TODO condense ported MappedData from Ghostwolf and MutableObjectData from
        // Retera

    }

    private void loadUnitsAndItems(final Warcraft3MapObjectData modifications, final War3MapW3i mapInformation) throws IOException {
        ...
        if (this.dataSource.has("war3mapUnits.doo") && WarsmashConstants.LOAD_UNITS_FROM_WORLDEDIT_DATA) {
            ...
            for (final com.etheller.warsmash.parsers.w3x.unitsdoo.Unit unit : dooFile.getUnits()) {
                ...
                final CWidget widgetCreated = createNewUnit(modifications, unitId, unitX, unitY, playerIndex, customTeamColor, unitAngle);
                if (widgetCreated instanceof CUnit) {
                    final CUnit unitCreated = (CUnit) widgetCreated;
                    if (editorConfigHitPointPercent > 0) {
                        unitCreated.setLife(this.simulation, unitCreated.getMaximumLife() * (editorConfigHitPointPercent / 100f));
                    }
    ...
    private CWidget createNewUnit(final Warcraft3MapObjectData modifications, final War3ID unitId, float unitX, float unitY, final int playerIndex, int customTeamColor, final float unitAngle) {
            ...
            if (row == null) {
                ...
                if (row != null) {
                    ...
                    if ((itemShadowFile != null) && !"_".equals(itemShadowFile)) {
                        ...
                        final float x = unitX - (float) itemShadowX;
                        final float y = unitY - (float) itemShadowY;
                        if (this.unitsReady) {
                            unitShadowSplatDynamicIngame = this.terrain.addUnitShadowSplat(texture, x, y, x + (float) itemShadowWidth, y + (float) itemShadowHeight, 3, 0.5f);
                        } else {
                            ...
                            this.terrain.splats.get(texture).locations.add(new float[]{x, y, x + (float) itemShadowWidth, y + (float) itemShadowHeight, 3});
                ...
                if (buildingPathingPixelMap != null) {
                    ...
                    pathingInstance = this.terrain.pathingGrid.blitRemovablePathingOverlayTexture(unitX, unitY, (int) Math.toDegrees(unitAngle), buildingPathingPixelMap);
                }
                ...
                if (uberSplat != null) {
                    ...
                    if (uberSplatInfo != null) {
                        final String texturePath = uberSplatInfo.getField("Dir") + "\\" + uberSplatInfo.getField("file") + ".blp";
                        ...
                        if (this.unitsReady) {
                            ...
                            buildingUberSplat.locations.add(new float[]{unitX - s, unitY - s, unitX + s, unitY + s, 1});
                        }
                    }
                }
                ...
                if ((unitShadow != null) && !"_".equals(unitShadow)) {
                    ...
                    if (this.mapMpq.has(texture)) {
                        ...
                        final float y = unitY - shadowY;
                        if (this.unitsReady) {
                            unitShadowSplatDynamicIngame = this.terrain.addUnitShadowSplat(texture, x, y, x + shadowWidth, y + shadowHeight, 3, 0.5f);
                        } else {
                            if (!this.terrain.splats.containsKey(texture)) {
                                final Splat splat = new Splat();
                                splat.opacity = 0.5f;
                            ...
                            this.terrain.splats.get(texture).locations.add(new float[]{x, y, x + shadowWidth, y + shadowHeight, 3});
                            unitShadowSplat = this.terrain.splats.get(texture);
                        }
                    }
                }
        ...
        if (path != null) {
            ...
            if (type == WorldEditorDataType.UNITS) {
                ...
                final CUnit simulationUnit = this.simulation.internalCreateUnit(row.getAlias(), playerIndex, unitX, unitY, angle, buildingPathingPixelMap, pathingInstance);
                ...
                final float unitZ = Math.max(getWalkableRenderHeight(unitX, unitY), War3MapViewer.this.terrain.getGroundHeight(unitX, unitY)) + simulationUnit.getFlyHeight();
                final RenderUnit renderUnit = new RenderUnit(this, model, row, unitX, unitY, unitZ, customTeamColor, soundset, portraitModel, simulationUnit, typeData, specialArtModel, buildingShadowInstance, this.selectionCircleScaleFactor, typeData.getAnimationWalkSpeed(), typeData.getAnimationRunSpeed(), typeData.getScalingValue());
                ...
                if (unitShadowSplat != null) {
                    unitShadowSplat.unitMapping.add(t -> renderUnit.shadow = t);
                }
                if (unitShadowSplatDynamicIngame != null) {
                    renderUnit.shadow = unitShadowSplatDynamicIngame;
                }
                if (buildingUberSplat != null) {
                    buildingUberSplat.unitMapping.add(t -> renderUnit.uberSplat = t);
                }
                ...
                final RenderItem renderItem = new RenderItem(this, model, row, unitX, unitY, unitZ, unitAngle, null, portraitModel, simulationItem);
                this.widgets.add(renderItem);
                this.itemToRenderPeer.put(simulationItem, renderItem);

                if (unitShadowSplat != null) {
                    unitShadowSplat.unitMapping.add(t -> renderItem.shadow = t);
                }
                if (unitShadowSplatDynamicIngame != null) {
    ...
    private BufferedImage getBuildingPathingPixelMap(final MutableGameObject row) {
        ...
        return loadPathingTexture(pathingTexture);
    }
            ...
            if (buildingPathingPixelMap == null) {
                try {
                    if (pathingTexture.toLowerCase().endsWith(".tga")) {
                        buildingPathingPixelMap = TgaFile.readTGA(pathingTexture, this.mapMpq.getResourceAsStream(pathingTexture));
                    } else {
    ...
    public RenderUnitTypeData getUnitTypeData(final War3ID key, final MutableGameObject row) {
        ...
        if (unitTypeData == null) {
            unitTypeData = new RenderUnitTypeData(row.getFieldAsFloat(MAX_PITCH, 0), row.getFieldAsFloat(MAX_ROLL, 0), row.getFieldAsFloat(ELEVATION_SAMPLE_RADIUS, 0), row.getFieldAsBoolean(ALLOW_CUSTOM_TEAM_COLOR, 0), row.getFieldAsInteger(TEAM_COLOR, 0), row.getFieldAsFloat(ANIMATION_RUN_SPEED, 0), row.getFieldAsFloat(ANIMATION_WALK_SPEED, 0), row.getFieldAsFloat(MODEL_SCALE, 0));
    ...
    public void update() {
        if (this.anyReady) {
            ...
            this.textTags.removeIf(textTag -> textTag.update(deltaTime));
            for (final RenderWidget unit : this.widgets) {
                unit.updateAnimations(this);
            }
            this.projectiles.removeIf(projectile -> projectile.updateAnimations(this, Gdx.graphics.getDeltaTime()));
            for (final RenderDoodad item : this.doodads) {
                ...
                if (instance instanceof MdxComplexInstance) {
                    ...
                    if ((mdxComplexInstance.sequence == -1) || (mdxComplexInstance.sequenceEnded && ((item.getAnimation() != AnimationTokens.PrimaryTag.DEATH) || (((MdxModel) mdxComplexInstance.model).sequences.get(mdxComplexInstance.sequence).getFlags() == 0)))) {
                        SequenceUtils.randomSequence(mdxComplexInstance, item.getAnimation(), SequenceUtils.EMPTY, true);
            ...
            if (this.dncTerrain != null) {
                this.dncTerrain.setFrameByRatio(this.simulation.getGameTimeOfDay() / this.simulation.getGameplayConstants().getGameDayHours());
                this.dncTerrain.update(rawDeltaTime, null);
            }
            if (this.dncUnit != null) {
                this.dncUnit.setFrameByRatio(this.simulation.getGameTimeOfDay() / this.simulation.getGameplayConstants().getGameDayHours());
                this.dncUnit.update(rawDeltaTime, null);
            }

            if (this.dncTarget != null) {
                this.dncTarget.setFrameByRatio(this.simulation.getGameTimeOfDay() / this.simulation.getGameplayConstants().getGameDayHours());
    ...
    public void doSelectUnit(final List<RenderWidget> units) {
        ...
        final Map<String, Terrain.Splat> splats = new HashMap<>();
        for (final RenderWidget unit : units) {
            if (unit.getSelectionScale() > 0) {
                ...
                if (!Objects.requireNonNull(path).toLowerCase().endsWith(".blp")) {
                    path += ".blp";
                }
                ...
                if (splatModel != null) {
                    ...
                    final SplatMover splatInstance = splatModel.add(x - (selectionSize / 2), y - (selectionSize / 2), x + (selectionSize / 2), y + (selectionSize / 2), 5, this.terrain.centerOffset);
                    ...
                    splats.get(path).locations.add(new float[]{x - (selectionSize / 2), y - (selectionSize / 2), x + (selectionSize / 2), y + (selectionSize / 2), 5});
                    splats.get(path).unitMapping.add(t -> {
        ...
        for (final Map.Entry<String, Terrain.Splat> entry : splats.entrySet()) {
            ...
            final SplatModel model = new SplatModel(Gdx.gl30, (Texture) load(filePath, PathSolver.DEFAULT, null), locations.locations, this.terrain.centerOffset, locations.unitMapping, true, false, true);
            switch (allyKey) {
                case "e:":
    ...
    public void showUnitMouseOverHighlight(final RenderWidget unit) {
        ...
        if (unit.getSelectionScale() > 0) {
            ...
            if (splatModel != null) {
                final float x = unit.getX();
                final float y = unit.getY();
                final SplatMover splatInstance = splatModel.add(x - (selectionSize / 2), y - (selectionSize / 2), x + (selectionSize / 2), y + (selectionSize / 2), 4, this.terrain.centerOffset);
                ...
                final float x = unit.getX();
                final float y = unit.getY();
                splats.get(path).locations.add(new float[]{x - (selectionSize / 2), y - (selectionSize / 2), x + (selectionSize / 2), y + (selectionSize / 2), 4});
                splats.get(path).unitMapping.add(t -> {
    ...
    public void getClickLocation(final Vector3 out, final int screenX, final int screenY) {
        ...
        RenderMathUtils.intersectRayTriangles(gdxRayHeap, this.terrain.softwareGroundMesh.vertices, this.terrain.softwareGroundMesh.indices, 3, out);
        ...
        this.walkableObjectsTree.intersect(rectangleHeap, this.walkablesIntersectionFinder.reset());
    ...
    public RenderWidget rayPickUnit(final float x, final float y, final CWidgetFilterFunction filter) {
        ...
        for (final RenderWidget unit : this.widgets) {
            ...
            if (instance.shown() && instance.isVisible(this.worldScene.camera) && instance.intersectRayWithCollisionSimple(gdxRayHeap, intersectionHeap)) {
                    ...
                    if (intersectionHeap.z > groundHeight) {
                        if (((entity == null) && !unit.isIntersectedOnMeshAlways())) {
                            ...
                            if (instance.intersectRayWithMeshSlow(gdxRayHeap, intersectionHeap)) {
                                if (intersectionHeap.z > this.terrain.getGroundHeight(intersectionHeap.x, intersectionHeap.y)) {
    ...
    public void setDayNightModels(final String terrainDNCFile, final String unitDNCFile) {
        ...
        final MdxModel targetDNCModel = loadModelMdx("Environment\\DNC\\DNCLordaeron\\DNCLordaeronTarget\\DNCLordaeronTarget.mdl");
    ...
    public void setGameUI(final GameUI gameUI) {
        this.gameUI = gameUI;
        this.abilityDataUI = new AbilityDataUI(this.allObjectData.getAbilities(), this.allObjectData.getBuffs(), this.allObjectData.getUnits(), this.allObjectData.getItems(), this.allObjectData.getUpgrades(), gameUI, this);
    }
    ...
    public RenderSpellEffect addSpecialEffectTarget(final String modelName, final CWidget targetWidget, final String attachPointName) {
        return addSpecialEffectTarget(modelName, targetWidget, Arrays.asList(attachPointName.split("\\s+")));
    }

    public RenderSpellEffect addSpecialEffectTarget(final String modelName, final CWidget targetWidget, List<String> attachPointNames) {
        if (targetWidget instanceof CUnit) {
            ...
            if (renderUnit == null) {
                throw new NullPointerException("renderUnit is null! targetWidget is \"" + ((CUnit) targetWidget).getUnitType().getName() + "\", attachPointName=\"" + attachPointNames + "\"");
            }
            ...
            if (spawnedEffectModel != null) {
                ...
                modelInstance.setScene(War3MapViewer.this.worldScene);
                final RenderSpellEffect renderAttackInstant = new RenderSpellEffect(modelInstance, War3MapViewer.this, yaw, RenderSpellEffect.DEFAULT_ANIMATION_QUEUE);
                War3MapViewer.this.projectiles.add(renderAttackInstant);
    ...
    public RenderSpellEffect addSpecialEffect(final String modelName, final float x, final float y) {
        ...
        if (spawnedEffectModel != null) {
            ...
            float yaw;
            {
                modelInstance.setLocation(x, y, Math.max(getWalkableRenderHeight(x, y), this.terrain.getGroundHeight(x, y)));
    ...
    public void setBlight(float whichLocationX, float whichLocationY, final float radius, final boolean blighted) {
        ...
        final Rectangle blightRectangle = new Rectangle(whichLocationX - radius, whichLocationY - radius, radius * 2, radius * 2);
        ...
        final Rectangle blightRectangleCellUnits = new Rectangle(cellMinX, cellMinY, cellMaxX - cellMinX, cellMaxY - cellMinY);
        this.terrain.updateGroundTextures(blightRectangleCellUnits);

        if (blighted) {
            this.simulation.getWorldCollision().enumDestructablesInRect(blightRectangle, destructable -> {
    ...
    private static final class StringDataCallbackImplementation implements LoadGenericCallback {
        ...
        public Object call(final InputStream data) {
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(data), StandardCharsets.UTF_8))) {
                String line;
    ...
    private static final class QuadtreeIntersectorFindsHitPoint implements QuadtreeIntersector<MdxComplexInstance> {
        ...
        private QuadtreeIntersectorFindsHitPoint reset() {
            this.ray = War3MapViewer.gdxRayHeap;
            this.found = false;
            return this;
        }
