...
public class Terrain {
    ...
    //
    public final List<GroundTexture> groundTextures = new ArrayList<>();
    public final List<UnloadedTexture> cliffTextures = new ArrayList<>();
    public final RenderCorner[][] corners;
    public final int columns;
    public final int rows;
    public final float[] maxDeepColor = new float[4];
    public final float[] minDeepColor = new float[4];
    public final float[] maxShallowColor = new float[4];
    public final float[] minShallowColor = new float[4];
    ...
    private final int groundTextureData;
    private int shadowMap;
    private boolean initShadowsFinished = false;
    private byte[] staticShadowData;
    private byte[] shadowData;

    public Terrain(final War3MapW3e w3eFile, final War3MapWpm terrainPathing, final War3MapW3i w3iFile, final WebGL webGL, final DataSource dataSource, final WorldEditStrings worldEditStrings, final War3MapViewer viewer, final DataTable worldEditData) throws IOException {
        this.webGL = webGL;
        this.viewer = viewer;
        ...
        for (int i = 0; i < (width - 1); i++) {
            for (int j = 0; j < (height - 1); j++) {
                ...
                bottomLeft.cliff = (bottomLeft.getLayerHeight() != bottomRight.getLayerHeight()) || (bottomLeft.getLayerHeight() != topLeft.getLayerHeight()) || (bottomLeft.getLayerHeight() != topRight.getLayerHeight());
            }
        }

        DataTable terrainTable = new DataTable(worldEditStrings);
        ...
        final Element waterInfo = waterTable.get(tileset + "Sha");
        if (waterInfo != null) {
        ...
        for (final Map.Entry<String, Integer> cliffVar : cliffVars.entrySet()) {
            final Integer maxVariations = cliffVar.getValue();
            for (int variation = 0; variation <= maxVariations; variation++) {
                final String fileName = "Doodads\\Terrain\\CityCliffs\\CityCliffs" + cliffVar.getKey() + variation + ".mdx";
                this.cliffMeshes.add(new CliffMesh(fileName, dataSource, Gdx.gl30));
        ...
        // Ground textures
        Map<String, Integer> groundTextureToId = new HashMap<>();
        for (final War3ID groundTile : w3eFile.getGroundTiles()) {
            final Element terrainTileInfo = terrainTable.get(groundTile.asStringValue());
            ...
            groundTextureToId.put(groundTile.asStringValue(), this.groundTextures.size() - 1);
        }

        final Element tilesets = worldEditData.get("TileSets");

        this.blightTextureIndex = this.groundTextures.size();
        this.groundTextures.add(new GroundTexture(tilesets.getField(Character.toString(tileset)).split(",")[1] + texturesExt, dataSource, Gdx.gl30));

        // Cliff Textures
        for (final War3ID cliffTile : w3eFile.getCliffTiles()) {
            final Element cliffInfo = cliffTable.get(cliffTile.asStringValue());
            if (cliffInfo == null) {
            ...
            final AnyExtensionImage imageInfo = ImageUtils.getAnyExtensionImageFixRGB(dataSource, texDir + "\\" + texFile + texturesExt, "cliff texture");
            final BufferedImage image = imageInfo.getRGBCorrectImageData();
            this.cliffTextures.add(new UnloadedTexture(image.getWidth(), image.getHeight(), ImageUtils.getTextureBuffer(image), cliffInfo.getField("cliffModelDir"), cliffInfo.getField("rampModelDir")));
            this.cliffTexturesSize = Math.max(this.cliffTexturesSize, this.cliffTextures.get(this.cliffTextures.size() - 1).width);
            this.cliffToGroundTexture.add(groundTextureToId.get(cliffInfo.getField("groundTile")));
        }
        ...
        byte[] waterExistsData = new byte[width * height];
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                waterExistsData[(j * width) + i] = (byte) this.corners[i][j].getWater();
        ...
        gl.glTexImage2D(GL30.GL_TEXTURE_2D, 0, GL30.GL_RGBA16UI, width - 1, height - 1, 0, GL30.GL_RGBA_INTEGER, GL30.GL_UNSIGNED_SHORT, RenderMathUtils.wrapShort(this.groundTextureList));
        ...
        gl.glTexImage2D(GL30.GL_TEXTURE_2D, 0, GL30.GL_R16F, width, height, 0, GL30.GL_RED, GL30.GL_FLOAT, RenderMathUtils.wrap(this.groundHeights));
        ...
        gl.glTexImage2D(GL30.GL_TEXTURE_2D, 0, GL30.GL_R16F, width, height, 0, GL30.GL_RED, GL30.GL_FLOAT, RenderMathUtils.wrap(this.groundCornerHeights));
        gl.glTexParameteri(GL30.GL_TEXTURE_2D, GL30.GL_TEXTURE_WRAP_S, GL30.GL_CLAMP_TO_EDGE);
        gl.glTexParameteri(GL30.GL_TEXTURE_2D, GL30.GL_TEXTURE_WRAP_T, GL30.GL_CLAMP_TO_EDGE);
        ...
        gl.glTexImage3D(GL30.GL_TEXTURE_2D_ARRAY, 0, GL30.GL_RGBA8, this.cliffTexturesSize, this.cliffTexturesSize, this.cliffTextures.size(), 0, GL30.GL_RGBA, GL30.GL_UNSIGNED_BYTE, null);
        ...
        for (final UnloadedTexture i : this.cliffTextures) {
            gl.glTexSubImage3D(GL30.GL_TEXTURE_2D_ARRAY, 0, 0, 0, sub, i.width, i.height, 1, GL30.GL_RGBA, GL30.GL_UNSIGNED_BYTE, i.data);
            sub += 1;
        }
        ...
        gl.glTexImage2D(GL30.GL_TEXTURE_2D, 0, GL30.GL_R16F, width, height, 0, GL30.GL_RED, GL30.GL_FLOAT, RenderMathUtils.wrap(this.waterHeights));
        gl.glTexParameteri(GL30.GL_TEXTURE_2D, GL30.GL_TEXTURE_MAG_FILTER, GL30.GL_NEAREST);
        gl.glTexParameteri(GL30.GL_TEXTURE_2D, GL30.GL_TEXTURE_MIN_FILTER, GL30.GL_NEAREST);
        ...
        gl.glTexImage2D(GL30.GL_TEXTURE_2D, 0, GL30.GL_R8, width, height, 0, GL30.GL_RED, GL30.GL_UNSIGNED_BYTE, RenderMathUtils.wrap(waterExistsData));
        ...
        final String fileName = Objects.requireNonNull(waterInfo).getField("texFile");
        ...
        for (int i = 0; i < this.waterTextureCount; i++) {
            final AnyExtensionImage imageInfo = ImageUtils.getAnyExtensionImageFixRGB(dataSource, fileName + (i < 10 ? "0" : "") + i + texturesExt, "water texture");
            final BufferedImage image = imageInfo.getImageData();
            if ((image.getWidth() != 128) || (image.getHeight() != 128)) {
                System.err.println("Odd water texture size detected of " + image.getWidth() + " x " + image.getHeight());
        ...
        gl.glTexImage3D(GL30.GL_TEXTURE_2D_ARRAY, 0, anyWaterTextureNeedsSRGB ? GL30.GL_SRGB8_ALPHA8 : GL30.GL_RGBA8, waterImageDimension, waterImageDimension, this.waterTextureCount, 0, GL30.GL_RGBA, GL30.GL_UNSIGNED_BYTE, null);
        ...
        for (int i = 0; i < waterTextures.size(); i++) {
            ...
            gl.glTexSubImage3D(GL30.GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, image.getWidth(), image.getHeight(), 1, GL30.GL_RGBA, GL30.GL_UNSIGNED_BYTE, ImageUtils.getTextureBuffer(image));
        }
        gl.glGenerateMipmap(GL30.GL_TEXTURE_2D_ARRAY);
        ...
        this.shaderMapBounds = new float[]{(this.mapBounds[0] * 128.0f) + this.centerOffset[0], (this.mapBounds[2] * 128.0f) + this.centerOffset[1], ((this.columns - this.mapBounds[1] - 1) * 128.0f) + this.centerOffset[0], ((this.rows - this.mapBounds[3] - 1) * 128.0f) + this.centerOffset[1]};
        this.shaderMapBoundsRectangle = new Rectangle(this.shaderMapBounds[0], this.shaderMapBounds[1], this.shaderMapBounds[2] - this.shaderMapBounds[0], this.shaderMapBounds[3] - this.shaderMapBounds[1]);
        this.mapSize = w3eFile.getMapSize();
        this.entireMapRectangle = new Rectangle(this.centerOffset[0], this.centerOffset[1], (this.mapSize[0] * 128f) - 128, (this.mapSize[1] * 128f) - 128);
        this.softwareGroundMesh = new SoftwareGroundMesh(this.groundHeights, this.groundCornerHeights, this.centerOffset, width, height);

        int testArrayBuffer = gl.glGenBuffer();
        gl.glBindBuffer(GL30.GL_ARRAY_BUFFER, testArrayBuffer);
        gl.glBufferData(GL30.GL_ARRAY_BUFFER, this.softwareGroundMesh.vertices.length, RenderMathUtils.wrap(this.softwareGroundMesh.vertices), GL30.GL_STATIC_DRAW);

        int testElementBuffer = gl.glGenBuffer();

        this.waveBuilder = new WaveBuilder(this.mapSize, waterTable, viewer, this.corners, this.centerOffset, this.waterHeightOffset, w3eFile, w3iFile);
            ...
            for (int i = (int) area.x; i < (area.x + area.width); i++) {
                ...
                for (int xOffset = -1; xOffset <= 0; xOffset++) {
                    for (int yOffset = -1; yOffset <= 0; yOffset++) {
                        if (((i + xOffset) >= 0) && ((i + xOffset) < (this.columns - 1)) && ((j + yOffset) >= 0) && ((j + yOffset) < (this.rows - 1))) {
                            ...
                            final int base = Math.min(Math.min(bottomLeft.getLayerHeight(), bottomRight.getLayerHeight()), Math.min(topLeft.getLayerHeight(), topRight.getLayerHeight()));
                ...
                corner.depth = (corner.getWater() != 0) ? (this.waterHeightOffset + corner.getWaterHeight()) - newGroundCornerHeight : 0;
            }
        }
        updateGroundHeights();
        updateCornerHeights();
    ...
    private void updateGroundHeights() {
        ...
        Gdx.gl30.glTexSubImage2D(GL30.GL_TEXTURE_2D, 0, 0, 0, this.columns, this.rows, GL30.GL_RED, GL30.GL_FLOAT, RenderMathUtils.wrap(this.groundHeights));
    }
    ...
    private void updateCornerHeights() {
        ...
        Gdx.gl30.glTexSubImage2D(GL30.GL_TEXTURE_2D, 0, 0, 0, this.columns, this.rows, GL30.GL_RED, GL30.GL_FLOAT, groundCornerHeightsWrapped);
        ...
        for (int i = (int) rampArea.getX(); i < xLimit; i++) {
            ...
            for (int j = (int) rampArea.getY(); j < yLimit; j++) {
                ...
                if (bottomLeft.cliff && !bottomLeft.hideCliff) {
                    ...
                    final boolean facingDown = (topLeft.getLayerHeight() >= bottomLeft.getLayerHeight()) && (topRight.getLayerHeight() >= bottomRight.getLayerHeight());
                    final boolean facingLeft = (bottomRight.getLayerHeight() >= bottomLeft.getLayerHeight()) && (topRight.getLayerHeight() >= topLeft.getLayerHeight());
                    ...
                    if (!(facingDown && (j == 0)) && !(!facingDown && (j >= (this.rows - 2))) && !(facingLeft && (i == 0)) && !(!facingLeft && (i >= (this.columns - 2)))) {
                        final boolean verticalRamp = ((bottomLeft.isRamp()) != (bottomRight.isRamp())) && ((topLeft.isRamp()) != (topRight.isRamp()));

                        final boolean horizontalRamp = ((bottomLeft.isRamp()) != (topLeft.isRamp())) && ((bottomRight.isRamp()) != (topRight.isRamp()));

                        final int tmp0 = (i + ((horizontalRamp ? 1 : 0) * (facingLeft ? -1 : 0)));
                        final int tmp1 = (j - ((verticalRamp ? 1 : 0) * (facingDown ? 1 : 0)));
                        final int tmp2 = i + ((facingLeft ? -1 : 1) * (horizontalRamp ? 1 : 0));
                        final int tmp3 = j + ((facingDown ? -1 : 1) * (verticalRamp ? 1 : 0));

                        if (verticalRamp || horizontalRamp) {
                            final boolean rampBlockedByCliff = ((verticalRamp && this.corners[i][j + (facingDown ? -1 : 1)].cliff) || (horizontalRamp && this.corners[i + (facingLeft ? -1 : 1)][j].cliff));
                            ...
                            if (!invalidRamp) {
                                ...
                                if (this.dataSource.has(fileName)) {
                                    ...
                                    for (int ji = this.cliffs.size(); ji-- > 0; ) {
                                        ...
                                        if ((pos.x == tmp0) && (pos.y == tmp1)) {
                                            this.cliffs.remove(ji);
                                            break;
                                        }
                                    }

                                    this.cliffs.add(new IVec3(tmp0, tmp1, this.pathToCliff.get(fileName)));
                                    ...
                                    this.corners[tmp2][tmp3].setCliffTexture(bottomLeftCliffTex);
                                    this.corners[tmp2][tmp3].romp = true;
                                    continue;
                                }
                            }
                        }
                    ...
                    fileName = this.cliffTextures.get(bottomLeftCliffTex).cliffModelDir + fileName + Variations.getCliffVariation(this.cliffTextures.get(bottomLeftCliffTex).cliffModelDir, fileName, bottomLeft.getCliffVariation());
                    if (!this.pathToCliff.containsKey(fileName)) {
    ...
    private void uploadGroundTexture() {
        if (this.groundTextureData != -1) {
            ...
            Gdx.gl30.glTexSubImage2D(GL30.GL_TEXTURE_2D, 0, 0, 0, this.columns - 1, this.rows - 1, GL30.GL_RGBA_INTEGER, GL30.GL_UNSIGNED_SHORT, RenderMathUtils.wrapShort(this.groundTextureList));
        }
    }
    ...
    private void getTextureVariations(final int x, final int y, final short[] out, final int outStartOffset) {
        ...
        out[outStartOffset] = (short) (firstValue + (getVariation(firstValue, this.corners[x][y].getGroundVariation()) << 5));

        int index;
        while (iterator.hasNext()) {
            index = 0;
            final int texture = iterator.next();
            index |= (bottomRight == texture ? 1 : 0);
                ...
                if (((x + i) >= 0) && ((x + i) < this.columns) && ((y + j) >= 0) && ((y + j) < this.rows)) {
                    if (this.corners[x + i][y + j].cliff) {
                        if (((x + i) < (this.columns - 1)) && ((y + j) < (this.rows - 1))) {
                            ...
                            if ((bottomLeft.isRamp()) && (topLeft.isRamp()) && (bottomRight.isRamp()) && (topRight.isRamp()) && (!bottomLeft.romp) && (!bottomRight.romp) && (!topLeft.romp) && (!topRight.romp)) {
                                break ILoop;
                            }
                        }
                    }
    ...
    private boolean isCornerRampEntrance(final int x, final int y) {
        ...
        return (bottomLeft.isRamp()) && (topLeft.isRamp()) && (bottomRight.isRamp()) && (topRight.isRamp()) && !((bottomLeft.getLayerHeight() == topRight.getLayerHeight()) && (topLeft.getLayerHeight() == bottomRight.getLayerHeight()));
    }
    ...
    public void renderGround(final DynamicShadowManager dynamicShadowManager) {
        ...
        gl.glUniformMatrix4fv(this.groundShader.getUniformLocation("MVP"), 1, false, this.camera.viewProjectionMatrix.val, 0);
        ...
        gl.glUniformMatrix4fv(this.groundShader.getUniformLocation("DepthBiasMVP"), 1, false, dynamicShadowManager.getDepthBiasMVP().val, 0);
...
//		gl.glEnableVertexAttribArray(0);
        ...
        gl.glDrawElementsInstanced(GL30.GL_TRIANGLES, Shapes.INSTANCE.quadIndices.length * 3, GL30.GL_UNSIGNED_INT, 0, (this.columns - 1) * (this.rows - 1));
        if (WIREFRAME_TERRAIN) {
...
//		gl.glDisableVertexAttribArray(0);
    ...
    public void renderCliffs() {
        ...
        for (final IVec3 i : this.cliffs) {
            ...
            final float min = Math.min(Math.min(bottomLeft.getLayerHeight() - 2, bottomRight.getLayerHeight() - 2), Math.min(topLeft.getLayerHeight() - 2, topRight.getLayerHeight() - 2));
...
//		gl.glActiveTexture(GL30.GL_TEXTURE2);
    ...
    public BuildingShadow addShadow(final String file, final float shadowX, final float shadowY) {
        ...
        if (this.initShadowsFinished) {
            ...
            final GL30 gl = Gdx.gl30;
            gl.glBindTexture(GL30.GL_TEXTURE_2D, this.shadowMap);
            gl.glTexImage2D(GL30.GL_TEXTURE_2D, 0, GL30.GL_R8, columns, rows, 0, GL30.GL_RED, GL30.GL_UNSIGNED_BYTE, RenderMathUtils.wrap(this.shadowData));
        }
    ...
    public void blitShadowData(final int columns, final int rows, final float shadowX, final float shadowY, final Texture texture) {
        final int width = texture.getWidth();
        final int height = texture.getHeight();
        final int ox = (int) Math.round(width * 0.3);
        final int oy = (int) Math.round(height * 0.7);
        blitShadowDataLocation(columns, rows, (RawOpenGLTextureResource) texture, width, height, ox, oy, this.centerOffset, shadowX, shadowY, this.shadowData);
    }
    ...
    public void initShadows() throws IOException {
        ...
        final int x0 = this.mapBounds[0] * 4, x1 = (this.mapSize[0] - this.mapBounds[1] - 1) * 4, y0 = this.mapBounds[2] * 4, y1 = (this.mapSize[1] - this.mapBounds[3] - 1) * 4;
        for (int y = y0; y < y1; ++y) {
            for (int x = x0; x < x1; ++x) {
    ...
    private void reloadShadowData(final float[] centerOffset, final int columns, final int rows) {
        ...
        for (final Map.Entry<String, Texture> fileAndTexture : this.shadowTextures.entrySet()) {
            ...
            for (final float[] location : this.shadows.get(file)) {
                blitShadowDataLocation(columns, rows, (RawOpenGLTextureResource) texture, width, height, ox, oy, centerOffset, location[0], location[1], this.shadowData);
            }
        }
    }

    public void blitShadowDataLocation(final int columns, final int rows, final RawOpenGLTextureResource texture, final int width, final int height, final int x01, final int y01, final float[] centerOffset, final float v, final float v2, final byte[] shadowData) {
    ...
    public void loadSplats() {
        for (final Map.Entry<String, Splat> entry : this.splats.entrySet()) {
            ...
            final SplatModel splatModel = new SplatModel(Gdx.gl30, (Texture) this.viewer.load(path, PathSolver.DEFAULT, null), splat.locations, this.centerOffset, splat.unitMapping.isEmpty() ? null : splat.unitMapping, false, false, false);
    ...
    public SplatMover addUberSplat(final String path, final float x, final float y, final float z, final float scale, final boolean unshaded, final boolean noDepthTest, final boolean highPriority) {
        SplatModel splatModel = this.uberSplatModels.get(path);
        if (splatModel == null) {
            splatModel = new SplatModel(Gdx.gl30, (Texture) this.viewer.load(path, PathSolver.DEFAULT, null), new ArrayList<>(), this.centerOffset, new ArrayList<>(), unshaded, noDepthTest, highPriority);
            addSplatBatchModel(path, splatModel);
        }
        return splatModel.add(x - scale, y - scale, x + scale, y + scale, z, this.centerOffset);
    }

    public SplatMover addUnitShadowSplat(final String texture, final float x, final float y, final float x2, final float y2, final float zDepthUpward, final float opacity) {
        SplatModel splatModel = this.uberSplatModels.get(texture);
        if (splatModel == null) {
            splatModel = new SplatModel(Gdx.gl30, (Texture) this.viewer.load(texture, PathSolver.DEFAULT, null), new ArrayList<>(), this.centerOffset, new ArrayList<>(), false, false, false);
    ...
    private void reloadShadowDataToGPU() {
        ...
        gl.glTexImage2D(GL30.GL_TEXTURE_2D, 0, GL30.GL_R8, columns, rows, 0, GL30.GL_RED, GL30.GL_UNSIGNED_BYTE, RenderMathUtils.wrap(Terrain.this.shadowData));
    }
    ...
    private static final class UnloadedTexture {
        ...
        public UnloadedTexture(final int width, final int height, final Buffer data, final String cliffModelDir, final String rampModelDir) {
            this.width = width;
            this.height = height;
            this.data = data;
    ...
    public static final class Splat {
        public final List<float[]> locations = new ArrayList<>();
        public final List<Consumer<SplatMover>> unitMapping = new ArrayList<>();
    ...
    public static final class SoftwareGroundMesh {
        ...
        private SoftwareGroundMesh(final float[] groundHeights, final float[] groundCornerHeights, final float[] centerOffset, final int columns, final int rows) {
            ...
            for (int y = 0; y < (rows - 1); y++) {
                for (int x = 0; x < (columns - 1); x++) {
                    ...
                    for (int vertexId = 0; vertexId < Shapes.INSTANCE.quadVertices.length; vertexId++) {
                        ...
                        this.vertices[(instanceId * 4 * 3) + (vertexId * 3) + 1] = ((vPositionY + y) * 128f) + centerOffset[1];
