...
public class CPathfindingProcessor {
    ...
    static void Init(final PathingGrid pathingGrid) {
        if (nodes == null) {
    ...
    public void removeFromPathfindingQueue(final CBehaviorMove behaviorMove) {
        ...
        this.moveQueue.removeIf(job -> job.queueItem == behaviorMove);
    ...
    public float h(final Node n) {
        ...
        for (int i = 0; i < this.goals; i++) {
            final float possibleDistance = (float) n.point.distance(goalSet[i].point);
    ...
    public void update(final CSimulation simulation) {
        ...
        while (!this.moveQueue.isEmpty()) {
            ...
            if (!job.jobStarted) {
                ...
                for (int i = goalCellX - 1; i <= (goalCellX + 1); i++) {
                    for (int j = goalCellY - 1; j <= (goalCellY + 1); j++) {
                        ...
                        if (possibleGoal.point.distance(job.goalX, job.goalY) <= bestGoalDistance) {
                            goalSet[this.goals++] = possibleGoal;
                        }
                    }
                }
                ...
                job.openSet = new PriorityQueue<>((a, b) -> Double.compare(f(a), f(b)));
                    ...
                    for (int cellY = job.startGridMinY; cellY <= job.startGridMaxY; cellY++) {
                        if ((cellX >= 0) && (cellX < this.pathingGrid.getWidth()) && (cellY >= 0)
                            ...
                            if (pathableBetween(job.ignoreIntersectionsWithThisUnit,
                                ...
                                possibleNode.g = possibleNode.point.distance(job.startX, job.startY);
                                job.openSet.add(possibleNode);

                            } else {
                                possibleNode.g = job.weightForHittingWalls;
                                job.openSet.add(possibleNode);
                ...
                if (isGoal(current)) {
                    ...
                    while (current.cameFrom != null) {
                        ...
                        if (lastCameFromDirection == null || current.cameFromDirection != lastCameFromDirection) {
                            if (current.cameFromDirection != null || !pathableBetween(job.ignoreIntersectionsWithThisUnit,
                                    job.ignoreIntersectionsWithThisSecondUnit, job.startX, job.startY,
                                    job.movementType, job.collisionSize, current.point.x, current.point.y) || !pathableBetween(job.ignoreIntersectionsWithThisUnit,
                                    job.ignoreIntersectionsWithThisSecondUnit, current.point.x, current.point.y,
                                    job.movementType, job.collisionSize, lastNode.point.x, lastNode.point.y) || !pathableBetween(job.ignoreIntersectionsWithThisUnit,
                                    job.ignoreIntersectionsWithThisSecondUnit, job.startX, job.startY,
                                    job.movementType, job.collisionSize, lastNode.point.x, lastNode.point.y) || !job.allowSmoothing) {
    ...
    public interface GridMapping {
        GridMapping CELLS = new GridMapping() {
            @Override
        ...
        GridMapping CORNERS = new GridMapping() {
            @Override
            public int getX(final PathingGrid grid, final float worldX) {
