...
import com.etheller.warsmash.viewer5.handlers.w3x.simulation.orders.*;
...
public class CUnit extends CWidget {
    private static final RegionCheckerImpl regionCheckerImpl = new RegionCheckerImpl();
    private final static AutoAttackTargetFinderEnum autoAttackTargetFinderEnum = new AutoAttackTargetFinderEnum();
    ...
    private final transient CUnitStateNotifier stateNotifier = new CUnitStateNotifier();
    private final transient List<StateListenerUpdate> stateListenersUpdates = new ArrayList<>();
    private final transient CBehaviorStop stopBehavior;
                    ...
                    if (this.buildQueueTypes[0] == QueueItemType.UNIT) {
                        ...
                        if (this.constructionProgress >= trainedUnitType.getBuildTime()) {
                            ...
                            if (this.rallyPoint != null) {
                                ...
                                this.rallyPoint.visit(
                                        UseAbilityOnTargetByIdVisitor.INSTANCE.reset(game, trainedUnit));
                            }
                        ...
                        if (this.constructionProgress >= gameplayConstants.getHeroReviveTime(
                            ...
                            if (this.rallyPoint != null) {
                                final int rallyOrderId = OrderIds.smart;
                                this.rallyPoint.visit(
                                        UseAbilityOnTargetByIdVisitor.INSTANCE.reset(game, revivingHero));
                            }
                            for (int i = 0; i < (this.buildQueue.length - 1); i++) {
                                setBuildQueueItem(game, i, this.buildQueue[i + 1], this.buildQueueTypes[i + 1]);
                            }
    ...
    public boolean order(final CSimulation simulation, final int orderId, final AbilityTarget target) {
        ...
        for (final CAbility ability : this.abilities) {
            ...
            if (activationReceiver.isOk()) {
                ...
                final boolean targetable = Objects.requireNonNull(target).visit(new AbilityTargetVisitor<Boolean>() {
                    @Override
            ...
            if (!sourcePlayer.hasAlliance(this.playerIndex, CAllianceType.PASSIVE)) {
                ...
                if (gameplayConstants.isBuildingKillsGiveExp() || !source.isBuilding()) {
                    ...
                    simulation.getWorldCollision().enumUnitsInRect(new Rectangle(getX() - heroExpRange,
                            getY() - heroExpRange, heroExpRange * 2, heroExpRange * 2), unit -> {
    ...
    private enum StateListenerUpdateType {
        ADD, REMOVE
    }
