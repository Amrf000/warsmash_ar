...
public class WarsmashGdxMapScreen implements InputProcessor, Screen {
    ...
    public WarsmashGdxMapScreen(final War3MapViewer mapViewer, final WarsmashGdxMultiScreenGame screenManager, final WarsmashGdxMenuScreen menuScreen, final CPlayerUnitOrderListener uiOrderListener) {
    ...
    public void show() {
        ...
        for (int i = 0; i < cameraPresets.length; i++) {
            cameraPresets[i] = new CameraPreset(cameraData.getFieldFloatValue("AOA", i), cameraData.getFieldFloatValue("FOV", i), cameraData.getFieldFloatValue("Rotation", i), cameraData.getFieldFloatValue("Rotation", i + cameraPresets.length), cameraData.getFieldFloatValue("Rotation", i + (cameraPresets.length * 2)), cameraData.getFieldFloatValue("Distance", i), cameraData.getFieldFloatValue("FarZ", i), cameraData.getFieldFloatValue("NearZ", i), cameraData.getFieldFloatValue("Height", i), cameraListenerData.getFieldFloatValue("ListenerDistance", i), cameraListenerData.getFieldFloatValue("ListenerAOA", i));
        }
...
//		this.consoleUITexture = new Texture(new DataSourceFileHandle(this.viewer.dataSource, "AlphaUi.png"));
        ...
        final CameraRates cameraRates = new CameraRates(cameraRatesElement.getFieldFloatValue("AOA"), cameraRatesElement.getFieldFloatValue("FOV"), cameraRatesElement.getFieldFloatValue("Rotation"), cameraRatesElement.getFieldFloatValue("Distance"), cameraRatesElement.getFieldFloatValue("Forward"), cameraRatesElement.getFieldFloatValue("Strafe"));
        this.meleeUI = new MeleeUI(this.viewer.mapMpq, this.uiViewport, this.uiScene, portraitScene, cameraPresets, cameraRates, this.viewer, WarsmashGdxMapScreen.this.viewer::setGameUI, this.uiOrderListener, () -> {
        ...
        final ModelInstance libgdxContentInstance = new LibGDXContentLayerModel(null, this.viewer, "", this.viewer.mapPathSolver, "").addInstance();
        ...
        CommonEnvironment commonEnv = Jass2.loadCommon(this.viewer.mapMpq, this.uiViewport, this.uiScene, this.viewer, this.meleeUI, "Scripts\\common.j", "Scripts\\abilitiesCommon.j", "Scripts\\Blizzard.j", "Scripts\\war3map.j");
        commonEnv.main();
    }
    ...
    public boolean touchDown(final int screenX, final int screenY, final int pointer, final int button) {
        ...
        this.meleeUI.touchDown(screenX, screenY, worldScreenY, button);
        return false;
    }
    ...
    public boolean touchUp(final int screenX, final int screenY, final int pointer, final int button) {
        ...
        this.meleeUI.touchUp(screenX, screenY, worldScreenY, button);
        return false;
    }
    ...
    public boolean touchDragged(final int screenX, final int screenY, final int pointer) {
        final float worldScreenY = this.viewer.canvas.getHeight() - screenY;
        this.meleeUI.touchDragged(screenX, screenY, worldScreenY, pointer);
        return false;
    }
    ...
    public boolean mouseMoved(final int screenX, final int screenY) {
        final float worldScreenY = this.viewer.canvas.getHeight() - screenY;
        this.meleeUI.mouseMoved(screenX, screenY, worldScreenY);
        return false;
    }
    ...
    private class LibGDXContentLayerModel extends Model<ModelHandler> {
