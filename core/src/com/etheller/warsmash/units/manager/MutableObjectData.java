        ...
        for (final String key : sourceSLKData.keySet()) {
            ...
            if (name.toString().startsWith("WESTRING")) {
                if (!name.toString().contains(" ")) {
                    name = new StringBuilder(this.worldEditStrings.getString(name.toString()));
                } else {
                    ...
                    for (final String subName : names) {
                        ...
                        if (subName.startsWith("WESTRING")) {
                            name.append(this.worldEditStrings.getString(subName));
                        } else {
                            name.append(subName);
                        }
                    }
...
//			final String newId = JOptionPane.showInputDialog("Choose UNIT ID");
            ...
            final MutableGameObject newObject = createNew(nextDefaultEditorId, entry.getValue().getOldId());
        ...
        this.changeNotifier.objectsCreated(newObjects.toArray(new War3ID[0]));
    }
    ...
    private MutableGameObject createNew(final War3ID id, final War3ID parent) {
    ...
    public void remove(final List<MutableGameObject> objects) {
        ...
        this.changeNotifier.objectsRemoved(removedIds.toArray(new War3ID[0]));
    }

    private void remove(final War3ID id, final boolean fireListeners) {
        if (this.cachedKeySet != null) {
...
//				this.parentWC3Object = new Element("", new DataTable());
        ...
        private void fireChangedEvent(final War3ID field) {
        ...
        public String getName() {
            StringBuilder name = new StringBuilder(getFieldAsString(MutableObjectData.this.editorData.getNameField(),
            ...
            if (!nameKnown) {
                name = new StringBuilder(MutableObjectData.this.worldEditStrings.getString("WESTRING_UNKNOWN") + " '"
                        + getAlias().toString() + "'");
            }
        ...
        private String getFieldStringFromSLKs(final War3ID field, final int level) {
            ...
            if (index != -1) {
                return this.parentWC3Object
                        .getField(getEditorMetaDataDisplayKey(level, metaData), index);
            }
            return this.parentWC3Object.getField(getEditorMetaDataDisplayKey(level, metaData));
        }
