...
import com.etheller.warsmash.parsers.fdf.datamodel.*;
...
public final class GameUI extends AbstractUIFrame implements UIFrame {
    ...
    public void hasSkinField(final String file) {
        if ((file != null)) {
            this.skin.hasField(file);
        }
    }
    ...
    public UIFrame createFrame(final String name, final UIFrame owner) {
    ...
    public UIFrame createSimpleFrame(final String name, final UIFrame owner) {
        final FrameDefinition frameDefinition = this.templates.getFrame(name);
    ...
    public BitmapFont generateFont(final float fdfFontSize) {
        ...
        return new LazyBitmapFont(this.fontGenerator, this.fontParam.size);
    }
        ...
        switch (frameDefinition.getFrameClass()) {
            case Frame:
                if ("SIMPLEFRAME".equals(frameDefinition.getFrameType())) {
                    ...
                    // TODO was fdfCoordinateResolutionDummyViewport here previously, but is that
                    // a good idea?
                    ...
                    frameFont = new LazyBitmapFont(this.dynamicFontGeneratorHolder.getFontGenerator(Objects.requireNonNull(font).getFontName()), this.fontParam.size);
                    ...
                    if (text != null) {
                        ...
                        if (!decoratedString.equals(text)) {
                    ...
                    for (final FrameDefinition childDefinition : frameDefinition.getInnerFrames()) {
                        if (childDefinition.getName().equals(controlBackdropKey)) {
                            ...
                            if (inflatedChild instanceof MenuFrame) {
                                ((MenuFrame) inflatedChild).setOnClick(glueButtonFrame::onClickItem);
                            }
                    ...
                    final Vector2Definition pushedTextOffset = frameDefinition.getVector2("ButtonPushedTextOffset");
                    if (pushedTextOffset != null) {
                        glueButtonFrame.setButtonPushedTextOffsetX();
                        glueButtonFrame.setButtonPushedTextOffsetY();
                    }
                    for (final FrameDefinition childDefinition : frameDefinition.getInnerFrames()) {
                        if (childDefinition.getName().equals(controlBackdropKey)) {
                            final UIFrame inflatedChild = inflate(childDefinition, glueButtonFrame, frameDefinition,
                    ...
                    final EditBoxFrame editBoxFrame = new EditBoxFrame(frameDefinition.getName(), parent,
                            editCursorColor);
                    // TODO: we should not need to put ourselves in this map 2x, but we do
                    // since there are nested inflate calls happening before the general case
                    // mapping
                    ...
                    // TODO advanced components here
                    final TextAreaFrame controlFrame = new TextAreaFrame(frameDefinition.getName(), parent);
                    // TODO: we should not need to put ourselves in this map 2x, but we do
                    // since there are nested inflate calls happening before the general case
                    // mapping
                    this.nameToFrame.put(frameDefinition.getName(), controlFrame);
                    ...
                    if (menuItemHeight != null) {
                        controlFrame.setItemHeight(convertY(viewport2, menuItemHeight));
                    }
            ...
            case String:
                ...
                inflatedFrame = new StringFrame(frameDefinition.getName(), parent, fontColor, justifyH,
    ...
    public UIFrame getFrameByName(final String name) {
