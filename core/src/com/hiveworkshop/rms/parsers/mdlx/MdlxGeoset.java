...
public class MdlxGeoset implements MdlxBlock, MdlxChunk {
    ...
    public long getByteLength(final int version) {
        long size = 120 + (this.vertices.length * 4L) + (this.normals.length * 4L) + (this.faceTypeGroups.length * 4L)
                + (this.faceGroups.length * 4L) + (this.faces.length * 2L) + this.vertexGroups.length
                + (this.matrixGroups.length * 4L) + (this.matrixIndices.length * 4L) + (this.sequenceExtents.size() * 28L);
        for (final float[] uvSet : this.uvSets) {
        ...
        if (version > 800) {
            ...
            if (this.tangents != null) {
                size += 8 + (this.tangents.length * 4L);
            }
