...
public abstract class AbstractFlacLowLevelInput implements FlacLowLevelInput {
    ...
    private static final char[] CRC16_TABLE = new char[256];
    ...
    private int readUnderlying() throws IOException {
        if (byteBufferIndex >= byteBufferLen) {
            ...
            updateCrcs(0);
            byteBufferLen = readUnderlying(byteBuffer, byteBuffer.length);
            crcStartIndex = 0;
            if (byteBufferLen <= 0)
                return -1;
            byteBufferIndex = 0;
    ...
    protected abstract int readUnderlying(byte[] buf, int len) throws IOException;
