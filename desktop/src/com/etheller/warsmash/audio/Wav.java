    ...
    static public class WavInputStream extends FilterInputStream {
        ...
        public WavInputStream(final InputStream stream, final Object loggableFile) {
            ...
            try {
                ...
                final int fmtChunkLength = seekToChunk('f', 'm', ' ');
                ...
                this.dataRemaining = seekToChunk('d', 'a', 'a');
        ...
        private int seekToChunk(final char c1, final char c2, final char c4) throws IOException {
            while (true) {
                ...
                final int chunkLength = (read() & 0xff) | ((read() & 0xff) << 8) | ((read() & 0xff) << 16)
                        | ((read() & 0xff) << 24);
                if (chunkLength == -1) {
                    throw new IOException("Chunk not found: " + c1 + c2 + 't' + c4);
                }
                if (found) {
                    return chunkLength;
                }
